...
/**
 * Class that tests the API functionality for obgyn office visits
 *
 * @author msned
 *
 */
...
@WebAppConfiguration
public class APIObstetricsControllerTest {
    private MockMvc               mvc;
    ...
    public void testObstetricsVisitAPI () throws Exception {
        ...
        visit.setType( AppointmentType.OBGYN_OFFICE_VISIT.toString() );
        visit.setHospital( "iTrust Test Hospital 2" );
        visit.setFundalHeight( 0.2 );
        visit.setFetalHeartRate( 100 );
        ...
        mvc.perform( get( "/api/v1/ObstetricsVisit/" + id ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) );

        visit.setDate( "2048-04-16T09:45:00.000-04:00" ); // 4/16/2048 9:45 AM

        mvc.perform( put( "/api/v1/ObstetricsVisit/" + id ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) );

        // PUT with the non-matching IDs should fail
        mvc.perform( put( "/api/v1/ObstetricsVisit/" + 1 ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isConflict() );

        // PUT with ID not in database should fail
        final long tempId = 101;
        visit.setId( "101" );
        mvc.perform( put( "/api/v1/ObstetricsVisit/" + tempId ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isNotFound() );

        // Reset ID to old id
        visit.setId( id.toString() );

        mvc.perform( delete( "/api/v1/ObstetricsVisit/" + id ) ).andExpect( status().isOk() );
